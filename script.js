...
if (leadForm) leadForm.addEventListener("submit", (event) => { event.preventDefault(); const data = new FormData(leadForm); const msg = [`Hola, quiero registrarme en Papeamigos.`, `Nombre: ${data.get("nombre") || ""}`, `Teléfono: ${data.get("telefono") || ""}`, `Estado: ${data.get("estado") || ""}`].join("\n"); window.open(wa(msg), "_blank", "noopener"); });
